...
> **章节数**：<N> 章 / <M> 步
> **节拍弧**：<情绪弧名称>（来自 rhythm.md，如：质疑 → 颠覆 → 证明 → 行动）

---

## 1. <chapter-id> — <章节标题>（<节拍类型> · <S> steps · ~<T>s）
...
### 章节标题：`## N. <id> — <title>（<节拍类型> · <S> steps · ~<T>s）`
...
| `<节拍类型>` | 来自 `rhythm.md`，如 `[hook]`、`[data]`、`[turn]` 等。chapter agent 据此约束视觉策略（见 [`RHYTHM-DESIGN.md`](RHYTHM-DESIGN.md) 节拍类型定义表） |
| `<S> steps` | 该章 step 总数 |
...
```
- step N (~Ts) — <屏幕内容> [img: <url>]（仅当该步对应 article 图片位置时）
```
...
| **`[img: url]`** | 当 article.md 在该段落位置有 `![](url)` 图片时，**必须**在对应 step 末尾标注 `[img: <完整url>]`。chapter agent 写代码时直接用此 URL，不靠猜测。一个 step 最多一张主图；若原文图片连续出现，各自对应独立 step |
...
**图片 step 示例**：
```
- step 3 (~4s) — 产品操作界面截图，全屏展示 [img: /api/projects/abc/assets/WVtp7By1VF.jpg]
- step 4 (~3s) — 段落要点文字（图片结束后回到文字画面）
```
...
- [ ] **article.md 中所有 `![](url)` 图片均已在对应 step 标注 `[img: url]`**，无遗漏——用 article.md 里的图片数量 cross-check outline 里的 `[img:]` 数量
