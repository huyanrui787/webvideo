...
interface Options {
  ...
  autoStarted: boolean;
  /** When true, pauses the current audio without destroying it. Resuming
   *  continues from the same position. Only meaningful in non-manual mode. */
  paused?: boolean;
  /** Playback speed multiplier. Default 1. Applied to the audio element. */
  playbackRate?: number;
}
...
export function useAudioPlayer({
  ...
  useEffect(() => {
    ...
    if (src) {
      ...
      audio.addEventListener("error", () => {
        ...
        if (mode === "auto") advanceAfter(estimateFallbackMs / playbackRate);
      });
      ...
      advanceAfter(estimateFallbackMs / playbackRate);
    }
  ...
  useEffect(() => {
    ...
    if (paused) {
      ...
      a.play().catch(() => {/* autoplay guard — ignore */});
    }
  }, [paused]);
