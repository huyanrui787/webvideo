/**
 * Image generation + fal.ai LipSync.
 */
...
/**
 * Generate a 1024×1024 image. Tries fal.ai first, falls back to gpt-image-2 proxy.
 * Returns raw PNG bytes as a Buffer.
 */
export async function generateImage(input: GenerateImageInput): Promise<GenerateImageResult> {
  // gpt-image-2 via proxy (primary)
  const key = process.env.GPT_IMAGE_KEY;
  const baseUrl = process.env.GPT_IMAGE_BASE_URL ?? "https://qqqapi.com";
  if (!key) throw new Error("No image generation API available");

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 600_000);
  const res = await fetch(`${baseUrl}/v1/images/generations`, { signal: controller.signal,
    method: "POST",
    headers: {
