...
const USE_CASES = [
  ...
  },
  {
    ...
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-brand-text">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {USE_CASES.map((uc, i) => (
            <ScrollReveal key={uc.title} delay={i * 100}>
              <Link
                href={uc.href}
                className={`group block rounded-2xl border border-bd ${uc.gradient} p-6 hover:border-amber-600/10 transition-all duration-300 h-full`}
              >
